...
public class GeminiProvider extends AbstractProvider {
    ...
    public String normal(List<Message> messages, String model) {
        try {
            return GptResponseBuilderUtil.normal(requestGemini());
        } catch (Exception e) {
            throw new RuntimeException("请求[" + getName() + "]异常", e);
        }
    }

    private String requestGemini() throws URISyntaxException, IOException, ParseException {
        ...
        try (CloseableHttpResponse response = httpclient.execute(httpPost)) {
            if (response.getCode() != 200) {
                throw new RuntimeException(
                    "请求[" + getName() + "]响应状态码异常,状态码为[" + response.getCode() + "]");
            }
